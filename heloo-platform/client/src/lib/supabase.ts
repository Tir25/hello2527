...
const customFetch = async (url: string | URL | Request, options?: RequestInit): Promise<Response> => {
  ...
  // FIX: Ensure proper headers for PostgREST content negotiation
  // This prevents 406 (Not Acceptable) errors caused by missing or incorrect headers
  const enhancedHeaders = new Headers(options?.headers as HeadersInit)

  // Ensure Accept header is set for JSON responses
  if (!enhancedHeaders.has('Accept')) {
    enhancedHeaders.set('Accept', 'application/json')
  }

  // Ensure Prefer header is set for proper response format with .single()/.maybeSingle()
  // This tells PostgREST to return representation and handle singular responses correctly
  if (!enhancedHeaders.has('Prefer') && !isAuthEndpoint) {
    enhancedHeaders.set('Prefer', 'return=representation')
  }
