...
export const useChat = () => {
  ...
  const fetchMessages = useCallback(
    async (receiverId: string, currentUserId: string) => {
      try {
        ...
        // Fetch messages with correct argument order: (otherUserId, currentUserId)
        const result = await chatService.fetchMessages(receiverId, currentUserId)
  ...
  useEffect(() => {
    ...
    socketService.onUserTyping((event) => {
      // Handle typing events for both DM and group chats

      // For DMs: Show typing indicator if the selected user is typing to us
      // event.userId = the person typing, event.receiverId = us
      // We show typing if the person typing (event.userId) is our selected conversation partner

      // For Groups: Show typing indicator if someone is typing in the selected group
      // event.groupId = the group where typing is happening
      // We show typing if the group matches our selected conversation

      const isGroupChat = !!event.groupId
      const isRelevantTyping = isGroupChat
        ? event.groupId === currentSelectedUser.id  // Group: match groupId to selected conversation
        : event.userId === currentSelectedUser.id   // DM: match userId to selected user

      if (isRelevantTyping) {
        // Look up the user's name from conversations or contacts for display
        let userName = 'Someone'

        // For DMs, use the selected user's name directly
        if (!isGroupChat && currentSelectedUser) {
          userName = currentSelectedUser.full_name || currentSelectedUser.username || 'Someone'
        } else {
          // For groups, look up the typing user from conversations or contacts
          const { conversations, contacts } = useChatStore.getState()

          // Check conversations first
          const typingUserConvo = conversations.find(c => c.id === event.userId && !c.is_group)
          if (typingUserConvo) {
            userName = typingUserConvo.full_name || typingUserConvo.username || 'Someone'
          } else {
            // Check contacts
            const typingUserContact = contacts.find(c => c.id === event.userId)
            if (typingUserContact) {
              userName = typingUserContact.full_name || typingUserContact.username || 'Someone'
            }
          }
        }

        // For groups, we want to show WHO is typing, not just that typing is happening
        // event.userId tells us WHO is typing in the group
        setUserTyping(event.userId, event.isTyping, userName)
      }
    })
