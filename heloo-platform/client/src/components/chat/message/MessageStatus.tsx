...
interface MessageStatusProps {
  ...
  isLastMessage?: boolean // MEDIUM #2: Only animate last message for performance
}
...
export const MessageStatus = ({ 
  status, 
  recipientAvatar,
  recipientThemeColor = 'rgb(139, 92, 246)', // Default purple/violet
  isLastMessage = false, // MEDIUM #2: Default to false to optimize performance
}: MessageStatusProps) => {
  const [avatarError, setAvatarError] = useState(false)

  // MEDIUM #2: Only animate if this is the last message (performance optimization)
  ...
  // Delivered: Two solid bubbles with bounce (HIGH #1: Fixed overlap)
  if (status === 'delivered') {
    return (
      <motion.div
      ...
      >
        ...
        />
        {/* Second bubble (offset) - properly sized */}
        <motion.div
          className="absolute w-2 h-2 rounded-full bg-white/80"
  ...
  return (
    ...
    >
      {/* Glow effect - pulsing outer ring (MEDIUM #2: Only animate if last message) */}
      <motion.div
      ...
      >
        {recipientAvatar && !avatarError ? (
          <img
            ...
            aria-hidden="true"
            onError={() => {
              // LOW #2: Fallback to theme color on avatar load error
              setAvatarError(true)
            }}
          />
        ) : (
          <div
            ...
            style={{ backgroundColor: recipientThemeColor }}
          />
        )}
      </motion.div>
    </motion.div>
